...
def get_json(url, sleep=0.6, retries=5, label="request"):
    """
    Slowed-down API helper.
    - sleep: delay before each request
    - retries: number of attempts
    - handles NHL rate limiting / temporary failures
    """
    for attempt in range(1, retries + 1):
        try:
            ...
            if r.status_code == 429:
                wait = 5 + attempt * 5
                print(f"Rate limited on {label}. Waiting {wait}s before retry {attempt}/{retries}...")
                time.sleep(wait)
                continue

            if r.status_code in [500, 502, 503, 504]:
                wait = 4 + attempt * 4
                print(f"NHL API temporary error {r.status_code} on {label}. Waiting {wait}s before retry {attempt}/{retries}...")
                time.sleep(wait)
                continue
            ...
            wait = 3 + attempt * 3
            print(f"Request failed on {label}. Waiting {wait}s before retry {attempt}/{retries}...")
            time.sleep(wait)
    ...
    try:
        ...
        if not game_log:
            ...
            if player_position == "G":
                ...
                shutouts = df.get("shutouts", pd.Series(dtype=float)).fillna(0).sum()
            else:
                wins = 0
                shutouts = 0
...
print("Fetching player playoff stats. This is the slow part...")

total_players = len(looper)
stats_rows = []

for i, (_, row) in enumerate(looper.iterrows(), start=1):
    stats_rows.append(get_player_stats(row, i=i, total=total_players))
...
joined["Player Eliminated"] = joined["Team"].isin(eliminated_teams)
...
def make_contestant_table_html(contestant, df):
    show_cols = [
        "Player", "Pos", "Team", "Goals", "Assists",
        "Wins", "Shutouts", "Points", "Player Eliminated"
    ]

    sub = df[df["Contestant"] == contestant].copy()
    sub = sub.sort_values(["rank_pos", "Points"], ascending=[True, False])
    sub = sub[show_cols].copy()

    total_points = int(sub["Points"].sum())
    remaining = int((~sub["Player Eliminated"]).sum())

    html = []
    html.append('<section class="team-card">')
    html.append(f"<h3>{esc(contestant)} <span>(Total Points: {total_points})</span></h3>")
    html.append(f'<p class="remaining">Players Remaining: {remaining}</p>')
    ...
    for _, row in sub.iterrows():
        ...
        for col in show_cols:
            ...
            if col in ["Wins", "Shutouts"] and row["Pos"] != "G" and int(value) == 0:
                value = "-"

            if col == "Player Eliminated":
                if bool(value):
                    html.append('<td class="eliminated">Eliminated</td>')
                else:
                    html.append('<td class="remaining-cell">Remaining</td>')
            elif col == "Points":
                html.append(f'<td class="points">{esc(value)}</td>')
            else:
...
<main>
  <div class="tabs">
    <button class="tab-button active" data-tab="leaderboard">Leader Board</button>
    <button class="tab-button" data-tab="teams">Playoff Pool Teams</button>
  </div>
  ...
  <p class="note">
    Scoring: skaters get goals + assists. Goalies get goals + assists + wins + three points per shutout.
  </p>
</main>
...
print("Upload nhl_pool_site.zip, or upload the contents of the nhl_pool_site folder.")
